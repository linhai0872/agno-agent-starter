...
class ToolCallGuard:
    ...
    def __init__(
        ...
        max_total_calls: int = 30,
        enabled: bool = True,
        config: Optional[ToolCallGuardConfig] = None,
    ):
        """
        初始化工具调用防护器
    ...
    def __call__(
        ...
        if total > self.config.max_total_calls:
            reason = (
                f"工具总调用次数 ({total}) 已超过上限 "
                f"({self.config.max_total_calls})"
            )
            logger.warning("ToolCallGuard: %s - forcing stop", reason)
            raise StopAgentRun(
                self.config.stop_message_template.format(reason=reason)
            )
        
        # 软限制: 单工具调用过多
        if current_count > self.config.max_calls_per_tool:
            self._retry_counter[function_name] = (
                self._retry_counter.get(function_name, 0) + 1
            )
...
_default_guard: Optional[ToolCallGuard] = None
_strict_guard: Optional[ToolCallGuard] = None
_relaxed_guard: Optional[ToolCallGuard] = None
